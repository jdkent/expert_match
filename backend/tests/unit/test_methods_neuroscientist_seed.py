...
def test_curated_methods_seed_has_twenty_unique_experts():
    ...
    assert all(expert.short_bio for expert in METHODS_NEUROSCIENTIST_EXPERTS)
    assert all(len(expert.short_bio) <= 500 for expert in METHODS_NEUROSCIENTIST_EXPERTS)
